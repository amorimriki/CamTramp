...
from services import buffer_manager, camera_manager, status_broadcaster, stream_manager
...
async def start_stream(camera_id: int) -> StreamStatus:
    ...
    # avisa já quem estiver ligado a /ws/status, sem esperar pelo próximo
    # tick do ciclo em segundo plano (ver services/status_broadcaster.py)
    await status_broadcaster.broadcast_now()
    return _status(camera_id, True)


@router.post("/{camera_id}/stream/stop", response_model=StreamStatus)
async def stop_stream(camera_id: int) -> StreamStatus:
